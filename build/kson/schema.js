...
/* Common objects */
function ByPulse(schema) {
    return z.tuple([Pulse, schema]);
}
function ByMeasureIdx(schema) {
    return z.tuple([MeasureIdx, schema]);
}
export const GraphValue = z.union([
    z.coerce.number().finite().transform((v) => [v, v]),
    z.tuple([z.coerce.number().finite()]).transform(([v]) => [v, v]),
...
export const GraphPoint = z.union([
    z.tuple([Pulse, GraphValue]).transform(([y, v]) => [y, v, [0, 0]]),
...
/* note */
export const ButtonNote = z.union([Pulse.transform((y) => [y, 0n]), z.tuple([Pulse, Pulse])]);
