import { NoteKind, btToNoteKind, fxToNoteKind, toLaserKind, PULSES_PER_WHOLE, } from "./type.js";
import Writer from "./writer.js";
const parseTimeSignature = (time_signature) => {
...
export default class Reader {
    ...
    audio_effects = [];
    _curr_pulse = 0n;
    _curr_pulses_per_measure = PULSES_PER_WHOLE;
    _curr_time_signature = [4, 4];
    _handleHeader(lines) {
        const carryovers = [];
    ...
    _setTimeSignature(numerator, denominator) {
    ...
    _handleMeasure(chart_line_count, lines) {
        ...
        for (const comment of curr_line.comments)
            carryovers.push(comment);
        for (const option of curr_line.options)
            carryovers.push(option);
        return carryovers;
    }
    _addUnknown(is_header, pulse, line) {
    ...
    static parse(chart_str) {
        ...
        for (const line_str of chart_str.split("\n")) {
            ...
            if (line.type === 'bar') {
                if (is_header)
                    chunk = reader._handleHeader(chunk);
                else
                    chunk = reader._handleMeasure(lines_per_measure, chunk);
                is_header = false;
        ...
        if (is_header)
            reader._handleHeader(chunk);
        else
            reader._handleMeasure(lines_per_measure, chunk);
        return reader;
    }
    /**
    ...
    static parseLine(line) {
        ...
        if (match) {
            return {
                type: match[1],
                name: match[2],
                params: match[3].split(';').map((param) => param.split('=', 2)),
            };
        }
        match = line.match(/^([012]{4})\|([012A-Z]{2})\|([0-9A-Za-o\-:]{2})(?:(@[()<>]|S[<>])(\d+))?$/);
        if (match) {
            const bt = match[1].split('').map(btToNoteKind);
            const legacy_fx = [null, null];
            const fx = match[2].split('').map((x, ind) => {
                const [note_kind, legacy] = fxToNoteKind(x);
                legacy_fx[ind] = legacy;
                return note_kind;
            });
            const laser = match[3].split('').map(toLaserKind);
            const line = {
